...
tabla_RelacionMedioTrans=table(prop.table(relacionMedioTrans_Genero))
...
# Grafico que permita analizar genero conformidad y tiempo usando tablas de contingencia

# Crear una tabla de contingencia entre genero, conformidad y tiempo
tabla_contingencia <- table(dr$`Genero `, dr$`Nivel de conformidad`, dr$`¿Qué tipo transporte usa?`)
...
# Renombrar las columnas
colnames(df_contingencia) <- c("Genero", "Conformidad", "Tipo_Transporte", "Frecuencia")

# Crear el grafico de barras
ggplot(df_contingencia, aes(x = Genero, fill = Conformidad)) +
  geom_bar(position = "dodge") +
  ...
  labs(
    ...
    fill = "Conformidad") +
  theme_minimal() +
  ylim(0, max(df_contingencia$Frecuencia))
```
